...
namespace MessageBar
{
	public class MessageView : UIView
	{
		...
		{
			BackgroundColor = UIColor.FromRGBA(0, 0, 0, 225);
			ClipsToBounds = false;
		...
		public override void LayoutSubviews()
		{
			if(_label == null)
			{
				_label = new UILabel(new CGRect(Padding, Padding / 2, Frame.Width - Padding * 2, Height - Padding));
				_label.AdjustsFontSizeToFitWidth = true;
				_label.Lines = 2;
				_label.LineBreakMode = UILineBreakMode.TailTruncation;
				_label.Font = UIFont.FromName("HelveticaNeue-Light", 16f);
		...
		CGRect GetStatusBarFrame()
		{
			...
			var width = windowFrame.Width > 768 ? 768 : windowFrame.Width;
			var x = (windowFrame.Width - width) / 2;

			return new CGRect(x, windowFrame.Y, width, statusFrame.Height);
		}
